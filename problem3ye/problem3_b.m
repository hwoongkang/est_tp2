...
F = [0 0; 0 0];         % Continuous System Matrix
Ts = 1;                 % Sampling Time
Phi = eye(2) + F*Ts;      % Discretized System Matrix
Gd = [1 0;0 1]*Ts;              
W = diag([1 1]).^2;  % km/s/sqrt(hz)
Wd=W/Ts;
...
for i=1:N-1
    ...
    x_est_temp=expm(F*Ts)*x_est_temp;
    M=Phi*P*Phi'+Gd*Wd*Gd';
